...
describe("SizeFilter2", () => {
  ...
  it("clears local input state after Clear All", () => {
    ...
    expect(currentContext().filters?.height).toBeUndefined()
    expect(currentContext().filters?.width).toBeUndefined()
  ...
  describe("the `expanded` prop", () => {
    ...
    it("hides the filter controls when `false`", () => {
      render(<SizeFilter2 expanded={false} />)
      expect(screen.queryByRole("checkbox")).not.toBeInTheDocument()
    })

    it("shows the filter controls when `true`", () => {
      render(<SizeFilter2 expanded={true} />)
